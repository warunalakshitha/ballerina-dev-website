...
>**Tip:** If you are still inside the previous `helloword` directory, use the `cd ..` command to go to the parent
> directory. Then, execute the `bal new winery` command to create the `winery` package. 
...
Execute the `bal add storage` command to create another `winery.storage` module to retrieve a list of wines from a
 storage. 
